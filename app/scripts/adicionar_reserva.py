...
from app.models import Hotels, Status, User, Reservation, Rooms, Guest, Account
...
def adicionar_reserva(user_id):
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            reservation = Reservation.query.filter((Reservation.room_id == form.room_id.data) & (
                    Reservation.check_in.between(form.check_in.data,
                                                 form.check_out.data) | Reservation.check_out.between(
                form.check_in.data, form.check_out.data)), Reservation.status == Status.ATIVO).all()
            if not reservation:
                ...
                if total_guests > room.guest_limit:
                    ...
                    conta = Account(tipo='Contas a receber',
    ...
    if request.method == 'GET':
        ...
        form.check_in.data = datetime.strptime(request.args.get('check_in'),
                                               "%Y-%m-%d") if 'check_in' in request.args else ''
        form.check_out.data = datetime.strptime(request.args.get('check_out'),
                                                "%Y-%m-%d") if 'check_out' in request.args else ''
...
def verificar_disponibilidade(user_id):
    ...
    if request.method == 'POST':
        if form_reserva.validate_on_submit():
            ...
            rooms_ids = db.session.query(Reservation.room_id).join(Rooms).filter(Rooms.hotel_id == user.hotel_id, (
                    Reservation.check_in.between(check_in, check_out) | Reservation.check_out.between(check_in,
                                                                                                      check_out)),
                                                                                 Reservation.status == Status.ATIVO)
            rooms = Rooms.query.filter(Rooms.hotel_id == user.hotel_id, Rooms.guest_limit >= total_guests,
                                       Rooms.id.not_in(rooms_ids))
...
def cancelar_reserva(reservation_id, user_id):
    form_reserva = VerificarDisponibilidade()
    user = User.query.filter_by(id=user_id).first()
    conta = Account.query.filter_by(reservation_id=reservation_id).first()

    if user.profile not in ['admin', 'gerente', 'recepcionista']:
        return '<h1>Erro! Você não pode acessar este conteúdo!</h1>'

    reservation = Reservation.query.join(Rooms).filter(Rooms.hotel_id == user.hotel_id,
