    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            reservation = Reservation.query.filter((Reservation.room_id == form.room_id.data) & (
                    ...
                    form.check_in.data, form.check_out.data))).all()
            if not reservation:
    ...
    if request.method == 'POST':
        if form_reserva.validate_on_submit():
            ...
            room = db.session.query(Rooms, Reservation) \
                .join(Reservation) \
                .filter((Reservation.room_id == form_reserva.room_id.data) & (
                        Reservation.check_in.between(form_reserva.check_in.data,
                                                     form_reserva.check_out.data) | Reservation.check_out.between(
                    form_reserva.check_in.data, form_reserva.check_out.data))) \
                .order_by(Reservation.check_in).all()
